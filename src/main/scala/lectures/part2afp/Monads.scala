...
object Monads extends App {
  ...
  /*
    ...
    Fail(e).flatMap(...) = Fail(e) // if you flatMap a Failure with anything you are still going to get a Failure
    ...
    Success(v).flatMap(v => f(v).flatMap(g)) = f(v).flatMap(g) OR Fail(e)
